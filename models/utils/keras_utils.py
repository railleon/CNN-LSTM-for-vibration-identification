...
def train_model(model:Model, dataset_id, dataset_prefix, dataset_fold_id=None, epochs=50, batch_size=128, val_subset=None,
    ...
    V_Train = tf.one_hot(y_train, depth=2, axis=1, dtype=tf.float32)
    V_Test = tf.one_hot(y_test, depth=2, axis=1, dtype=tf.float32)

    session = tf.Session()
    y_train = session.run(V_Train)
    y_test = session.run(V_Test)
...
def evaluate_model(model:Model, dataset_id, dataset_prefix, dataset_fold_id=None, batch_size=128, test_data_subset=None,
                   cutoff=None, normalize_timeseries=False, four_input= False, predict=False):
    ...
    if predict is True:
        ...
        pre_label2 = [np.argmax(one_hot) for one_hot in result]
        pre_label= np.array(pre_label2)

        np.savetxt('predicted label.txt', pre_label, fmt='%1.9f')
        ...
        ture_label2 = [np.argmax(one_hot) for one_hot in result2]

        ture_label  = np.array(ture_label2 )

        np.savetxt('correct label.txt', ture_label, fmt='%1.9f')

    else:
        pre_label=[]
        ture_label =[]

    return accuracy, loss, pre_label,ture_label
